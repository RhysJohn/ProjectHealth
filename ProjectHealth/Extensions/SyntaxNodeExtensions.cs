...
namespace ProjectHealth.Extensions;

public static class SyntaxNodeExtensions
{
    internal static int CalculateCyclomaticComplexity(this SyntaxNode node)
    {
        var complexity = 1; // Start with 1 for the root node

        // Count 'if' statements
        complexity += node.DescendantNodes().OfType<IfStatementSyntax>().Count();

        // Count 'else if' statements
        complexity += node.DescendantNodes().OfType<ElseClauseSyntax>().Count();

        // Count 'case' statements in switch statements
        complexity += node.DescendantNodes().OfType<SwitchSectionSyntax>()
            .SelectMany(section => section.Labels as IEnumerable<SwitchLabelSyntax>)
            .Count(label => label.Keyword.IsKind(SyntaxKind.CaseKeyword));

        // Count 'while', 'for', 'foreach', and 'do-while' loops
    ...
    internal static StatementSyntax EmbeddedStatement(this UsingStatementSyntax usingStatement,
        bool allowUsingStatement = true)
    {
